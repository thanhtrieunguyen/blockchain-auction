...
const VerificationStatus = ({ verifierContract, tokenId, account, onRequestVerification }) => {
  ...
  const normalizeTokenId = (id) => {
    if (typeof id === 'string') {
      // Chuyển đổi string thành số nếu có thể
      return id.trim() === '' ? '0' : id.trim();
    }
    return String(id);
  };
  ...
  const checkVerificationStatus = async () => {
    try {
      ...
      console.log(`Kiểm tra trạng thái xác thực cho token ID: ${tokenIdValue}`);

      // Kiểm tra xem NFT đã được xác thực chưa
      const isVerified = await verifierContract.methods.isNFTVerified(tokenIdValue).call();
      console.log(`Token #${tokenIdValue} verification status:`, isVerified);
      ...
      if (isVerified) {
        ...
        try {
          ...
          console.warn("Không thể lấy lý do xác thực:", error);
        }
      } else {
        ...
        try {
          ...
          console.log(`Yêu cầu xác thực cho token #${tokenIdValue}:`, request);
    ...
    try {
      ...
      await verifierContract.methods.requestVerification(tokenIdValue)
        ...
        .then(receipt => {
          ...
          // Cập nhật trạng thái sau khi yêu cầu thành công
          setStatus(1); // pending
        });
    } catch (error) {
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Button
              ...
              disabled={loading}
            >
              {loading ? <CircularProgress size={16} /> : 'Yêu cầu xác thực'}
            </Button>
          </Box>
        );
    }
